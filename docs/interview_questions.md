...
### 🔹 Что такое `WeakMap`?
**`WeakMap`** — это коллекция **пар ключ-значение**, где **ключами могут быть только объекты** или `Symbol`.  
Главная особенность — **слабые ссылки на ключи**, которые позволяют сборщику мусора удалять ключи, если на них нет других ссылок.

### 🔥 **Отличия `WeakMap` от `Map`**
| Особенность  | `Map` | `WeakMap` |
|-------------|------|-----------|
| Тип ключей  | Любые (`string`, `number`, `object`) | Только **объекты** или `Symbol` |
| Удаление сборщиком мусора | ❌ **Нет** (ключ остаётся навсегда) | ✅ **Да** (если нет ссылок, ключ удаляется) |
| Перебор элементов (`forEach`, `keys`, `values`, `entries`) | ✅ **Есть** | ❌ **Нет** |
| Количество элементов (`size`) | ✅ **Есть** | ❌ **Нет** |

📌 `WeakMap` помогает **избежать утечек памяти** при работе с временными объектами.
...
| `has(key)` | Проверяет, есть ли ключ в `WeakMap` |
| `delete(key)` | Удаляет ключ и его значение |

⚠ **Нет методов для перебора элементов!** (`size`, `keys()`, `values()`, `entries()`, `forEach()` и т. д.)

---

## 🔹 **Примеры работы с `WeakMap`**

### 📌 Создание `WeakMap` и добавление объектов
```javascript
...
wm.set(obj1, "Данные о пользователе Alice");
wm.set(obj2, "Данные о пользователе Bob");

console.log(wm.get(obj1)); // "Данные о пользователе Alice"
console.log(wm.has(obj2)); // true
```
📌 **Ключи в `WeakMap` – только объекты**, примитивные значения (`string`, `number`, `boolean`) **нельзя использовать**.

---

### 📌 Удаление ключа и проверка наличия
```javascript
wm.delete(obj1);
console.log(wm.has(obj1)); // false (объект удалён)
```
...
usersData.set(user, "Некоторые данные");

console.log(usersData.has(user)); // true

user = null; // Теперь на объект нет ссылок
// В какой-то момент объект будет удалён сборщиком мусора
```
📌 **Объект `user` исчезнет из `WeakMap`, когда на него не останется ссылок**.  

**В `Map` удалённый объект остался бы в памяти навсегда!**

---

## 🔹 **Когда использовать `WeakMap`?**
### ✅ **Отслеживание данных, связанных с объектами**
Можно хранить метаданные о DOM-элементах или временных объектах, не препятствуя их удалению.

```javascript
const elementData = new WeakMap();

let div = document.createElement("div");
elementData.set(div, "Информация о div");

console.log(elementData.get(div)); // "Информация о div"

div.remove(); // Элемент удалён из DOM
div = null; // Теперь сборщик мусора может удалить объект
```

📌 **В `Map` удалённый элемент остался бы в памяти!**  

---

## 📌 Итоги
✅ **`WeakMap` – это коллекция объектов** с данными, которые могут исчезнуть.  
✅ **Ключами могут быть только объекты или `Symbol`** (примитивы не подходят).  
✅ **Объекты автоматически удаляются, если на них нет ссылок** (слабые ссылки).  
✅ **Нет методов перебора, нет свойства `size`**.  
✅ **Полезен для хранения временных данных без риска утечек памяти**.  

`WeakMap` –  инструмент для **работы с временными объектами** и **избежания утечек памяти**. 🚀
